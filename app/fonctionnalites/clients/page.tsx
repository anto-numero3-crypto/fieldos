...
export default function ClientsPage() {
  ...
  const features = [
    ...
    { icon: Phone, label: fr ? 'Coordonnées et adresses' : 'Contact info and addresses' },
    ...
    { icon: Search, label: fr ? 'Recherche et filtres avancés' : 'Advanced search and filters' },
    { icon: Upload, label: fr ? 'Import CSV en lot' : 'Bulk CSV import' },
    { icon: Tag, label: fr ? 'Tags et catégories' : 'Tags and categories' },
  ]

  const steps = [
    { num: '1', title: fr ? 'Ajoutez un client' : 'Add a client', desc: fr ? 'Entrez les coordonnées ou importez votre liste existante en CSV.' : 'Enter contact info or import your existing list via CSV.' },
    { num: '2', title: fr ? 'Consignez les interactions' : 'Log interactions', desc: fr ? 'Chaque intervention, facture et note est liée automatiquement au profil.' : 'Every job, invoice, and note is automatically linked to the profile.' },
    { num: '3', title: fr ? 'Accédez à l\'historique partout' : 'Access history anywhere', desc: fr ? 'Consultez le dossier complet depuis votre téléphone ou votre ordinateur.' : 'View the full file from your phone or computer.' },
  ]

  const faqs = [
    {
      q: fr ? 'Puis-je importer mes clients depuis Excel ?' : 'Can I import my clients from Excel?',
      a: fr ? 'Oui. Exportez votre fichier Excel en CSV et importez-le directement dans Gestivio. Les doublons sont détectés automatiquement.' : 'Yes. Export your Excel file as CSV and import it directly into Gestivio. Duplicates are detected automatically.',
    },
    {
      q: fr ? 'Y a-t-il une limite au nombre de clients ?' : 'Is there a limit to the number of clients?',
      a: fr ? 'Non. Vous pouvez ajouter autant de clients que nécessaire, sans limite et sans frais supplémentaires.' : 'No. You can add as many clients as you need, with no limit and no extra fees.',
    },
    {
      q: fr ? 'Puis-je voir tout l\'historique d\'un client ?' : 'Can I see a client\'s full history?',
      a: fr ? 'Oui. Le profil client affiche toutes les interventions, factures, paiements et notes associés en un seul endroit.' : 'Yes. The client profile shows all associated jobs, invoices, payments, and notes in one place.',
    },
    {
      q: fr ? 'Comment fonctionnent les tags ?' : 'How do tags work?',
      a: fr ? 'Créez des tags personnalisés (ex. VIP, résidentiel, commercial) et assignez-les à vos clients pour filtrer rapidement.' : 'Create custom tags (e.g., VIP, residential, commercial) and assign them to clients for quick filtering.',
    },
  ]
  ...
  const related = [
    ...
    { href: '/fonctionnalites/import', label: fr ? 'Import de données' : 'Data Import' },
  ]
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="mt-6 text-lg text-gray-600 leading-relaxed max-w-xl">
                {fr ? 'Fini les infos dispersées dans des carnets et fichiers Excel. Centralisez tout sur chaque client en un seul endroit accessible partout.' : 'No more info scattered across notebooks and Excel files. Centralize everything about each client in one place, accessible anywhere.'}
              </p>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="mt-4 text-gray-600 leading-relaxed">
                {fr ? 'Numéros de téléphone dans un carnet, adresses dans un fichier, historique dans votre tête. Quand un client appelle, vous cherchez partout.' : 'Phone numbers in a notebook, addresses in a file, history in your head. When a client calls, you search everywhere.'}
              </p>
            </div>
            <div>
              <h2 className="text-3xl font-bold text-gray-900">
                {fr ? 'Après : tout en un seul clic' : 'After: everything in one click'}
              </h2>
              <p className="mt-4 text-gray-600 leading-relaxed">
                {fr ? 'Ouvrez le profil du client et voyez ses coordonnées, ses interventions passées, ses factures et vos notes. Tout est là.' : 'Open the client profile and see their contact info, past jobs, invoices, and your notes. Everything is there.'}
              </p>
            </div>
          </div>
        </div>
